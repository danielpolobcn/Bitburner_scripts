...
### Hacknet (min / prof)
Both scripts will manage the constant purchase and upgrade of hacknet nodes on behalf of the player. Thge <b>min</b> version will attempt to buy the next cheapest node or upgrade, while the <b>prof</b> will purchase the next most profitable option (defined as delta-income divided by cost of purchase). The scripts will take two arguments to set the maximum number of nodes to buy and the delay (in seconds) between every new attempt to purchase, or default to 12 nodes and 10 seconds if no arguments are passed.
